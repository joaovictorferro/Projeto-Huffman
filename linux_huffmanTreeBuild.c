...
void Compressing(FILE *FileToCompress, FILE *FileBits, hash *hashTable, long int *Setted_Bits){
  unsigned char character, currentByte = 0;
  short int size = 0;
  ...
  while( fscanf(FileToCompress,"%c",&character) != EOF ){
    ...
    while( hashTable->items[character]->binaryCode[i] != '\0' ){
      (*Setted_Bits) ++;
      if(size==8){
        fprintf(FileBits,"%c",currentByte);
        size = 0;
        currentByte = 0;
      }
      if(hashTable->items[character]->binaryCode[i] & 1){
        currentByte = SetBit(currentByte,size);
      }
      size ++;
      i++;
    }
